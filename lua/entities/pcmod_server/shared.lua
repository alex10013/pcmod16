...
ENT.Class = "pcmod_server"

ENT.ItemModel = "models/props/de_nuke/equipment1.mdl"
...
function ENT:Setup( setupdata )
	...
	table.insert( pts, self:CreatePort( "cat5e" ) )
	dt.Ports = pts
...
function ENT:DataRecieved( port, data )
	...
	if (prt.Type == "cat5e") then self:PushDriverData( "gen_network", port, data ) end
