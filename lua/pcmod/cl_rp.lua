...
function PCMod.RP.OpenBuyMenu()
	...
	if (LocalPlayer():Team() != TEAM_COMPTECH) then 
	net.Start( "Nope" )
...
function PCMod.RP.BuyHardware( itemid )
	...
	if (name == "Tower") then
		...
		vgui.AddButton( pl, "Choose Tower", function( self )
			local model = DermaMenu()
            model:AddOption( "New Tower", function() btn.Mdl = "models/props/cs_office/computer_caseb.mdl" end )
			model:AddOption( "Old Tower", function() btn.Mdl = "models/props_lab/harddrive02.mdl" end )
			model:Open()
		end )
		PCMod.RP.OSID = cb
    end
	...
	if (name == "Monitor") then
        vgui.AddButton( pl, "Choose Monitor", function( self )
			local model = DermaMenu()
            model:AddOption( "New Monitor", function() btn.Mdl = "models/props/cs_office/computer_monitor.mdl" end )
			model:AddOption( "Old Monitor", function() btn.Mdl = "models/props_lab/monitor01a.mdl" end )
			model:Open()
		end )
	end
	
	if (name == "Keyboard") then
        vgui.AddButton( pl, "Choose Keyboard", function( self )
			local model = DermaMenu()
            model:AddOption( "New Keyboard", function() btn.Mdl = "models/props/cs_office/computer_keyboard.mdl" end )
			model:AddOption( "Old Keyboard", function() btn.Mdl = "models/props_c17/computer01_keyboard.mdl" end )
			model:Open()
		end )
	end
	
	if (name == "Printer") then
        vgui.AddButton( pl, "Choose Printer", function( self )
			local model = DermaMenu()
            model:AddOption( "New Printer", function() btn.Mdl = "models/props_lab/plotter.mdl" end )
			model:AddOption( "Old Printer", function() btn.Mdl = "models/pcmod/kopierer.mdl" end )
			model:Open()
		end )
	end
