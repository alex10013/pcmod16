...
function SWEP:PasteHD()
	...
	PCMod.Notice( "You must be looking at an object!", self.Owner  ) 
		return
	end
	if ((!ent.IsPCMod) || (ent:GetClass() != "pcmod_tower") and (ent:GetClass() != "pcmod_laptop")) then 
	PCMod.Notice( "You must be looking at a PC Tower!", self.Owner  ) 
		return
	end
	...
	local osid2 = hd[ "/system/data/allprogs.dat" ]
	if (!osid2) then osid2 = {} end
	...
	local tabloyusay = table.Count(PCMod.StringToTable( osid2.ItemContent))
	
	for i=1, tabloyusay do
	local lazim = PCMod.StringToTable( osid2.ItemContent)[i][3]
	local islenmis = string.Replace( lazim, "sys_openprog ", "" )
	ent:InstallProgram( islenmis )
	end
...
function SWEP:EraseSlot()
	local sl = self.SelSlot
	if (PLD) then dat = PCMod.PLD.GetPlyData( self.Owner ) end
	if (!dat) then dat = {} end
	if (!dat.HDrives) then dat.HDrives = {} end
	...
	if (ss) then
		...
		if (PLD) then PCMod.PLD.SetPlyData( self.Owner, dat ) end
